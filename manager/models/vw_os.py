...
class ViewOrders(BaseModel):
    ...
    def get_expireds(cls, cr, data):
        ...
        oss = ViewOrders.query
        oss = oss.filter(
            ViewOrders.cr == cr,
            ViewOrders.situacao.in_(["ABERTA", "ORÇAMENTO"]),
            func.to_char(cast(ViewOrders.abertura, Date), 'MM-YYYY') >= data
        ).order_by(ViewOrders.abertura.desc()).all()
